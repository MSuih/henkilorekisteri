...
/** Alustaa derby-tietokannan. Luo kaikki taulut ja täyttää ne tiedoilla.*/
public class AlustaDerbyTietokanta {
    private AlustaDerbyTietokanta() {}
    
    //<editor-fold defaultstate="collapsed" desc="luontilauseet">
    private static final String[] lauseet = new String[]{
        
        "CREATE TABLE mahdollisetroolit ( " +
        ...
        "  FOREIGN KEY työntekijäID (työntekijäID))",*/
        ...
        "CREATE TABLE työrooli (" +
        "  työrooliID int NOT NULL GENERATED ALWAYS AS IDENTITY (START WITH 1, INCREMENT BY 1)," +
        "  työntekijäID char(4) NOT NULL," +
        "  työpaikkaID char(4) NOT NULL," +
        "  rooliID char(4) NOT NULL," +
        "  kuvaus varchar(500) DEFAULT NULL," +
        "  PRIMARY KEY (työrooliID), "
            + "FOREIGN KEY (rooliID) REFERENCES mahdollisetroolit(rooliID))",
        ...
        "ALTER TABLE työajat" +
        "  ADD CONSTRAINT työajat_ibfk_1 FOREIGN KEY (työntekijäID) REFERENCES työntekijä (työntekijäID) ON DELETE NO ACTION ON UPDATE RESTRICT",
        
        "ALTER TABLE työrooli" +
        "  ADD CONSTRAINT työrooli_ibfk_1 FOREIGN KEY (työntekijäID) REFERENCES työntekijä (työntekijäID) ON DELETE NO ACTION ON UPDATE RESTRICT",
        
        "ALTER TABLE työrooli" +
        "  ADD CONSTRAINT työrooli_ibfk_2 FOREIGN KEY (työpaikkaID) REFERENCES työpaikka (työpaikkaID) ON DELETE NO ACTION ON UPDATE RESTRICT",
        
        "ALTER TABLE työrooli" +
        "  ADD CONSTRAINT työrooli_ibfk_3 FOREIGN KEY (rooliID) REFERENCES mahdollisetroolit (rooliID) ON DELETE NO ACTION ON UPDATE RESTRICT"
            };
//</editor-fold>
    
    /** Alusta tietokanta.
     * @throws java.util.zip.DataFormatException Jos luontilauseet eivät toimineet*/
    public static void alusta() throws DataFormatException {
        
        Connection yhteys = null;
        try {
            File f = new File("henkilotietokanta");
            
            Class.forName("org.apache.derby.jdbc.EmbeddedDriver").newInstance();
            yhteys = DriverManager.getConnection("jdbc:derby:henkilotietokanta;create=true;characterEncoding=UTF-8", "tunnus", "sala");
            try (Statement lause = yhteys.createStatement()) {
                ResultSet vastaus = lause.executeQuery("select * from sys.sysschemas");
                boolean b = false;
                while (vastaus.next()) {
                    String s = vastaus.getString(2);
                ...
                if (!b) {
                    try (Statement luo = yhteys.createStatement()) {
                        luo.executeUpdate("create schema tunnus");
                    }
                }
            }
            for (String s : lauseet) {
                System.out.println(s.substring(0, 25));
                try (Statement l = yhteys.createStatement()) {
                    l.execute(s);
                }
            ...
            JOptionPane.showMessageDialog(null, "Tietokanta on käyttövalmis", "Ilmoitus", JOptionPane.INFORMATION_MESSAGE);
